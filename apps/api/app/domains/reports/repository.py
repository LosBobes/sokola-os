...
SQLAlchemy models directly. Every query is filtered by ``school_id`` (or,
...
def active_member_count(db: Session, school_id: str) -> int:
...
def active_group_membership_count_as_of(
    db: Session, school_id: str, group_id: str, as_of: dt.datetime
) -> int:
...
def collected_total(
    ...
    stmt = select(func.coalesce(func.sum(PaymentRecord.amount), 0)).where(
        PaymentRecord.school_id == school_id,
...
def outstanding_debt_total(db: Session, school_id: str) -> Decimal:
    stmt = select(
