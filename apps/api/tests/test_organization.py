...
def test_create_school_makes_caller_owner(client: TestClient, db: Session) -> None:
    ...
    # The creator now has an OWNER context for that school.
    contexts = client.get("/me/contexts", headers=headers).json()
    match = [c for c in contexts if c["school_id"] == org["id"]]
    assert len(match) == 1
    assert match[0]["role_code"] == "OWNER"


def test_get_current_school_uses_context(client: TestClient, db: Session) -> None:
    from tests.factories import bootstrap_actor

    actor = bootstrap_actor(db, org_name="Klub Soko")
    resp = client.get("/schools/current", headers=actor.headers)
    assert resp.status_code == 200
    assert resp.json()["id"] == actor.school.id
...
def test_tenant_lookup_by_slug(client: TestClient, db: Session) -> None:
    ...
    assert found.json()["school_id"] == created["id"]
