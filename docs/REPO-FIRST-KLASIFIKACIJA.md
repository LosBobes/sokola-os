...
| M03 Multi-tenancy | `ADAPT` | §5.2 + TEN-04/TEN-05 isporučeni: `tenant_security_state` sa `tenant_access_version`, atomski sa M04 statusnom promenom (`app/domains/tenancy/`, migracija `e5f83a19d24b`, 17 testova). **Ostaje:** `SessionTenantContext` §5.3 i §8 korak 3 (F-25), kontekst po role assignment-u umesto škola+workspace (F-24, čeka M05), composite tenant FK 4/46 (F-26), `student_login_authorization` globalan (F-27) |
...
### F-25 — `TenantSecurityState` isporučen; `SessionTenantContext` i verzije konteksta ostaju (DELIMIČNO OTKLONJENO)
...
- **Šta ostaje:** `SessionTenantContext` (§5.3) i poređenje `context_version` / `authorization_version` / `tenant_access_version` iz §8 koraka 3. Verzija sada **postoji**, ali je još niko ne poredi — nema gde, dok ne postoji red konteksta po sesiji.
- **Status:** `DELIMIČNO OTKLONJENO` — §5.2 i TEN-04/TEN-05 gotovi; §5.3 i §8 korak 3 su sledeći isečak.
...
- **Repo dokaz (prebrojano nad `Base.metadata`):** 61 tabela ukupno, 46 sa `school_id`, i **4** composite tenant FK ograničenja — `participant_profile`, `staff_profile` (oba `(school_id, school_membership_id, membership_type)`), `school_owner_nomination` i `school_primary_owner_term`. Sva četiri su iz M04/M06 rada u ovom talasu; sve starije veze koriste običan FK na `id`.
- **Posledica:** za veliku većinu relacija cross-tenant referenca je sprečena **samo aplikacionim guardom**. To nije ništa — guard postoji i testiran je — ali §7.3 traži da to sprovodi baza, jer aplikacioni guard koji neko jednom zaboravi ne ostavlja trag.
- **Zašto nije urađeno odjednom:** 42 tabele × (`UNIQUE(school_id, id)` na parentu + prepisan FK na detetu) je migracija koja dodiruje skoro celu šemu. To je izvodljivo, ali kao sopstveni PR sa sopstvenim probama, a ne kao usputna izmena.
- **Minimalni predlog:** redom po talasima — prvo Foundation tabele (school_membership, role_assignment, school_person_profile), pa Operational Core. Svaki korak sa testom koji zaobilazi servis i pokušava cross-tenant upis direktno kroz SQL.
- **Status:** `CHALLENGE_NOT_APPLIED` — zabeleženo kao poznat, merljiv dug sa brojem: 4/46.
...
- **Status:** `DELIMIČNO OTKLONJENO` — `workspace` je **obrisan** (migracija `e5f83a19d24b`; dokazano bez čitalaca, pa `DROP TABLE`, ne migracija podataka). `student_login_authorization` ide u M03/M09 isečak, `guardian_relationship` čeka M07 presudu.
