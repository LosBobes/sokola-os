...
| M03 Multi-tenancy | `ADAPT` | Server-derived context postoji i testiran je (`app/security/{context,deps}.py`), ali ga bira **role assignment header**, ne škola+workspace — vidi F-24. Nema `TenantSecurityState`, `SessionTenantContext` ni `tenant_access_version` (F-25). Composite tenant FK ima samo 4 od 46 tenant tabela (F-26). Tri globalne tabele nose tenant odluke (F-27) |
...
### F-25 — nema `TenantSecurityState`, `SessionTenantContext` ni verzija konteksta
...
- **Status:** `CHALLENGE_NOT_APPLIED` — prvi M03 isečak, i prirodno prvi jer ostatak §8 zavisi od njega.
...
- **Status:** `CHALLENGE_NOT_APPLIED` — `student_login_authorization` ide u M03/M09 isečak, `guardian_relationship` čeka M07, a `workspace` se briše zajedno sa prvim M03 isečkom (dokazano bez čitalaca, pa je to `DROP TABLE`, ne migracija podataka).
